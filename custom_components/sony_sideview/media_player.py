"""Media player platform for Sony SideView devices."""
...
async def async_setup_entry(
    ...
    async_add_entities([SonySideViewMediaPlayer(coordinator, entry)])


class SonySideViewMediaPlayer(
    CoordinatorEntity[SonyDataUpdateCoordinator], MediaPlayerEntity
):
    """Representation of a Sony SideView media player."""

    _attr_has_entity_name = True
    _attr_name = None
    _attr_device_class = MediaPlayerDeviceClass.RECEIVER
    _attr_supported_features = SUPPORT_SONY

    def __init__(
        self, coordinator: SonyDataUpdateCoordinator, entry: ConfigEntry
    ) -> None:
        super().__init__(coordinator)
        self._entry = entry
        host = entry.data[CONF_HOST]
        self._attr_unique_id = f"sony_sideview_{host}_media_player"
